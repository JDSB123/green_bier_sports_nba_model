...
async def test_the_odds_api():
    ...
    try:
        ...
        if games and len(games) > 0:
            ...
            print(
                f"  [OK] Sample game: {games[0].get('home_team', 'N/A')} vs {games[0].get('away_team', 'N/A')}"
            )
            results.append(
                {"endpoint": "/sports/basketball_nba/odds", "status": "pass", "count": len(games)}
            )
        else:
            print(f"  [WARN] No games returned (may be off-season)")
            results.append(
                {"endpoint": "/sports/basketball_nba/odds", "status": "warn", "count": 0}
            )
        ...
        if "401" in error_msg or "403" in error_msg or "unauthorized" in error_msg:
            ...
            results.append(
                {"endpoint": "/sports/basketball_nba/odds", "status": "fail", "error": "auth"}
            )
        else:
            print(f"  [FAIL] Error: {e}")
            results.append(
                {"endpoint": "/sports/basketball_nba/odds", "status": "fail", "error": str(e)}
            )
    ...
    try:
        ...
        if events and len(events) > 0:
            ...
            results.append(
                {"endpoint": "/sports/basketball_nba/events", "status": "warn", "count": 0}
            )
    except Exception as e:
        error_msg = str(e).lower()
        if "401" in error_msg or "403" in error_msg:
            ...
            results.append(
                {"endpoint": "/sports/basketball_nba/events", "status": "fail", "error": "auth"}
            )
        else:
            print(f"  [FAIL] Error: {e}")
            results.append(
                {"endpoint": "/sports/basketball_nba/events", "status": "fail", "error": str(e)}
            )
    ...
    try:
        ...
        if scores and len(scores) > 0:
            ...
            results.append(
                {"endpoint": "/sports/basketball_nba/scores", "status": "warn", "count": 0}
            )
    except Exception as e:
        error_msg = str(e).lower()
        if "401" in error_msg or "403" in error_msg:
            print(f"  [FAIL] Authentication failed")
            results.append(
                {"endpoint": "/sports/basketball_nba/scores", "status": "fail", "error": "auth"}
            )
        else:
            print(f"  [FAIL] Error: {e}")
            results.append(
                {"endpoint": "/sports/basketball_nba/scores", "status": "fail", "error": str(e)}
            )
    ...
    for name, fetch_fn in tier1_endpoints:
        ...
        try:
            ...
            if result and result.count > 0:
                ...
                results.append(
                    {"endpoint": f"/{name}", "status": "pass", "count": result.count, "tier": 1}
                )
            else:
            ...
            if "401" in error_msg or "403" in error_msg or "unauthorized" in error_msg:
                print(f"    [FAIL] Authentication failed - check API key")
                results.append(
                    {"endpoint": f"/{name}", "status": "fail", "error": "auth", "tier": 1}
                )
            elif "429" in error_msg or "rate limit" in error_msg:
                print(f"    [WARN] Rate limited - may need to wait")
                results.append(
                    {"endpoint": f"/{name}", "status": "warn", "error": "rate_limit", "tier": 1}
                )
            else:
                print(f"    [FAIL] Error: {e}")
                results.append(
                    {"endpoint": f"/{name}", "status": "fail", "error": str(e), "tier": 1}
                )
