...
from src.utils.historical_guard import resolve_historical_output_root
...
OUTPUT_FILE = resolve_historical_output_root(
        ...
        try:
            ...
            if len(dfs) > 4:
                ...
                if len(line_score) == 2:  # Home and away
                    ...
                    home_row = line_score[line_score["teamId"] == game_info.get(
                        "homeTeamId")].iloc[0] if "homeTeamId" in game_info else line_score.iloc[1]
                    away_row = line_score[line_score["teamId"] == game_info.get(
                        "awayTeamId")].iloc[0] if "awayTeamId" in game_info else line_score.iloc[0]
...
def main():
    ...
    print(df[["game_date", "home_team", "away_team",
          "home_1h", "away_1h", "1h_total_actual"]].head(5))
