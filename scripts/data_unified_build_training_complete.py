...
from src.data import CST, generate_match_key, standardize_team_name
...
def load_kaggle(start_date: str) -> pd.DataFrame:
    ...
    df.loc[home_1h_mask, "home_1h"] = (
        df.loc[home_1h_mask, "home_q1"] + df.loc[home_1h_mask, "home_q2"]
    )
    df.loc[~home_1h_mask, "home_1h"] = np.nan
    df.loc[away_1h_mask, "away_1h"] = (
        df.loc[away_1h_mask, "away_q1"] + df.loc[away_1h_mask, "away_q2"]
    )
    ...
    df.loc[home_2h_mask, "home_2h"] = (
        df.loc[home_2h_mask, "home_q3"] + df.loc[home_2h_mask, "home_q4"]
    )
    df.loc[~home_2h_mask, "home_2h"] = np.nan
    df.loc[away_2h_mask, "away_2h"] = (
        df.loc[away_2h_mask, "away_q3"] + df.loc[away_2h_mask, "away_q4"]
    )
...
def load_theodds_2025_26() -> pd.DataFrame:
    ...
    for idx, row in df.iterrows():
        ...
        h1_ml_home_vals.append(
            row.get(h2h_h1_home_col) if h2h_h1_home_col in df.columns else np.nan
        )
        h1_ml_away_vals.append(
            row.get(h2h_h1_away_col) if h2h_h1_away_col in df.columns else np.nan
        )

        # Q1 moneyline (h2h_q1)
        h2h_q1_home_col = f"h2h_q1_{home_raw}_price"
        h2h_q1_away_col = f"h2h_q1_{away_raw}_price"
        q1_ml_home_vals.append(
            row.get(h2h_q1_home_col) if h2h_q1_home_col in df.columns else np.nan
        )
        q1_ml_away_vals.append(
            row.get(h2h_q1_away_col) if h2h_q1_away_col in df.columns else np.nan
        )
...
def load_line_movement() -> pd.DataFrame:
    ...
    for match_key, group in df.groupby("match_key"):
        ...
        spread_rows_home = group[
            (group["market_key"] == "spreads") & (group["outcome_name"] == home_team)
        ]
        spread_rows_away = group[
            (group["market_key"] == "spreads") & (group["outcome_name"] == away_team)
        ]
        ...
        total_rows_over = group[
            (group["market_key"] == "totals") & (group["outcome_name"] == "Over")
        ]
        total_rows_under = group[
            (group["market_key"] == "totals") & (group["outcome_name"] == "Under")
        ]
        ...
        h1_spread_rows_home = group[
            (group["market_key"] == "spreads_h1") & (group["outcome_name"] == home_team)
        ]
        h1_spread_rows_away = group[
            (group["market_key"] == "spreads_h1") & (group["outcome_name"] == away_team)
        ]
        ...
        h1_total_rows_over = group[
            (group["market_key"] == "totals_h1") & (group["outcome_name"] == "Over")
        ]
        h1_total_rows_under = group[
            (group["market_key"] == "totals_h1") & (group["outcome_name"] == "Under")
        ]
        ...
        movements.append(
            {
                ...
                "spread_move": (
                    ...
                    if pd.notna(open_spread) and pd.notna(close_spread)
                    else None
                ),
                ...
                "total_move": (
                    ...
                    if pd.notna(open_total) and pd.notna(close_total)
                    else None
                ),
...
def load_box_scores() -> pd.DataFrame:
    ...
    for side in ["home", "away"]:
        ...
        df[f"{side}_efg_pct"] = (df[f"fgm_{side}"] + 0.5 * df[f"fg3m_{side}"]) / df[
            f"fga_{side}"
        ].replace(0, np.nan)
        ...
        df[f"{side}_tov_pct"] = df[f"tov_{side}"] / (
            df[f"fga_{side}"] + 0.44 * df[f"fta_{side}"] + df[f"tov_{side}"]
        ).replace(0, np.nan)
        df[f"{side}_oreb_pct"] = df[f"oreb_{side}"] / (
            df[f"oreb_{side}"] + df[f"dreb_{opp}"]
        ).replace(0, np.nan)
        df[f"{side}_poss"] = (
            df[f"fga_{side}"] - df[f"oreb_{side}"] + df[f"tov_{side}"] + 0.44 * df[f"fta_{side}"]
        )
...
def load_nba_api_box_scores() -> pd.DataFrame:
    ...
    for side in ["home", "away"]:
        ...
        result[f"{side}_oreb_pct"] = result[f"{side}_oreb"] / (
            result[f"{side}_oreb"] + result[f"{opp}_dreb"]
        ).replace(0, np.nan)
        result[f"{side}_poss"] = (
            fga - result[f"{side}_oreb"] + result[f"{side}_to"] + 0.44 * result[f"{side}_fta"]
        )

    for side in ["home", "away"]:
        opp = "away" if side == "home" else "home"
        result[f"{side}_off_rtg"] = (
            result[f"{side}_score"] / result[f"{side}_poss"].replace(0, np.nan) * 100
        )
        result[f"{side}_def_rtg"] = (
            result[f"{opp}_score"] / result[f"{opp}_poss"].replace(0, np.nan) * 100
        )
...
def merge_all(
    kaggle, theodds, theodds_2526, h1_exp, movement, box_old, box_new, quarters=None
) -> pd.DataFrame:
        ...
        if not new_games.empty:
            ...
            for _, row in new_games.iterrows():
                new_row = {
                    ...
                    "game_date": (
                        pd.to_datetime(row.get("commence_time", row.get("game_date"))).tz_localize(
                            None
                        )
                        if pd.notna(row.get("commence_time", row.get("game_date")))
                        else None
                    ),
    ...
    if not theodds.empty:
        ...
        theodds_merge = theodds[[c for c in cols if c in theodds.columns]].drop_duplicates(
            "match_key"
        )
...
def compute_fg_features(df: pd.DataFrame, min_games: int = 3) -> pd.DataFrame:
        ...
        team_rows.append(
            {
                ...
                "margin": margin,
                "win": (
                    1 if pd.notna(margin) and margin > 0 else (0 if pd.notna(margin) else np.nan)
                ),
            }
        )
        team_rows.append(
    ...
    home_feats = team_df[team_df["side"] == "home"][
        ["game_idx", "ppg", "papg", "avg_margin", "win_pct"]
    ].rename(
        columns={
            "game_idx": "_game_idx",
            "ppg": "home_ppg",
    ...
    away_feats = team_df[team_df["side"] == "away"][
        ["game_idx", "ppg", "papg", "avg_margin", "win_pct"]
    ].rename(
        columns={
            "game_idx": "_game_idx",
...
def standardize_core_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for target, cands in [
        ("1h_spread_line", h1_spread_candidates),
        ("1h_total_line", h1_total_candidates),
    ]:
        if target not in df.columns:
            df[target] = pd.NA
        for c in cands:
    ...
    for target, cands in [
        ("q1_spread_line", q1_spread_candidates),
        ("q1_total_line", q1_total_candidates),
    ]:
        if target not in df.columns:
            df[target] = pd.NA
        for c in cands:
...
def compute_situational(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, r in df.iterrows():
        ...
        team_streaks[h] = (
            max(1, team_streaks.get(h, 0) + 1) if win else min(-1, team_streaks.get(h, 0) - 1)
        )
        team_streaks[a] = (
            max(1, team_streaks.get(a, 0) + 1) if not win else min(-1, team_streaks.get(a, 0) - 1)
        )
...
def print_summary(df: pd.DataFrame):
    ...
    for name, col in [
        ...
        ("Line movement", "spread_move"),
        ("Box scores", "home_efg_pct"),
        ("ELO", "home_elo"),
    ]:
        if col in df.columns:
...
def _download_prebuilt_training_data(
    blob_account: str = "nbagbsvstrg", version: str = "latest"
) -> Path:
    ...
    Args:
        ...
        version: Version to download (e.g., "latest", "v2026.01.14")

    Returns:
        Path to downloaded training_data.csv
    """
    import tempfile
    from pathlib import Path
    ...
    try:
        sas_url = _get_blob_sas_url(blob_account, container, blob_path)
        cmd = ["azcopy", "copy", sas_url, str(output), "--overwrite=ifSourceNewer"]
        result = subprocess.run(cmd, capture_output=True, text=True, check=True)
        ...
        print(f"  [PREBUILT] Falling back to az cli...")

        # Fallback to az storage blob download
        try:
            cmd = [
                "az",
                "storage",
...
def _get_blob_sas_url(blob_account: str, container: str, blob_path: str) -> str:
    """Generate SAS URL for blob access (for azcopy)."""
    # For simplicity, use az cli to generate SAS URL
    # In production, you'd use connection strings or managed identity
    try:
        ...
        return f"https://{blob_account}.blob.core.windows.net/{container}/{blob_path}?{sas_token}"
    except Exception as e:
        raise RuntimeError(f"Failed to generate SAS URL: {e}") from e
...
def main(
    ...
    # If --use-prebuilt flag is set, skip all raw data collection
    if use_prebuilt:
        print("\n[PREBUILT] Downloading validated training data from Azure...")
        try:
            _download_prebuilt_training_data(blob_account, prebuilt_version)
            print("[PREBUILT] Training data ready. Skipping raw data rebuild.")
    ...
    if skip_post_processing:
        ...
        subprocess.run(
            [
                sys.executable,
                str(PROJECT_ROOT / "scripts" / "data_unified_feature_complete.py"),
                "--training-file",
                str(out),
            ],
            check=True,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--skip-post-processing", action="store_true", help="Skip gap fixes and feature completion"
    )
    parser.add_argument(
        "--seasons",
