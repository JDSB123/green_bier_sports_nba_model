...
class FirstQuarterSpreadFilter:
    ...
    v6.5: Q1 filter with proportionally scaled danger zone.
    Q1 is ~25% of FG, so danger zone is 0.75-1.5 points (vs FG 3-6).
    """

    def __init__(
        self,
        filter_small_spreads: bool = True,
        small_spread_min: float = 0.75,
        small_spread_max: float = 1.5,
        min_edge_pct: float = 0.05,
        min_confidence: float = 0.60,  # Stricter for Q1
    ):
        """
        Initialize first quarter spread filter.

        Args:
            filter_small_spreads: Remove 0.75-1.5 point spreads (proportional to FG 3-6)
            small_spread_min: Minimum spread to filter
            small_spread_max: Maximum spread to filter
            min_edge_pct: Minimum model edge required (default 5%)
            min_confidence: Minimum confidence required (stricter for Q1)
        """
        self.filter_small_spreads = filter_small_spreads
        self.small_spread_min = small_spread_min
        self.small_spread_max = small_spread_max
    ...
    def should_bet(
        ...
        """
        Determine if a first quarter spread bet should be made.

        Args:
            spread_line: Vegas Q1 spread line (absolute value)
            confidence: Model confidence (0-1)
        ...
        # Filter 1: Minimum confidence (stricter for Q1)
        if confidence < self.min_confidence:
            return False, f"Low Q1 confidence ({confidence:.1%} < {self.min_confidence:.0%})"

        # Filter 2: Small spreads (0.75-1.5 points for Q1)
        if self.filter_small_spreads:
            if self.small_spread_min <= spread_abs <= self.small_spread_max:
        ...
        if model_edge_pct < self.min_edge_pct:
            return False, f"Insufficient Q1 edge ({model_edge_pct:.1%} < {self.min_edge_pct:.0%})"
