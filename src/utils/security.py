...
def validate_premium_features() -> Dict[str, Dict[str, any]]:
    ...
    features = {
        "odds_primary": {
            ...
            "provides": ["spreads", "totals", "period markets (1H)"],
        },
        "game_data": {
    ...
    required_missing = [
        f["name"] for f in features.values() if f["required"] and not f["configured"]
    ]
    ...
    optional_missing = [
        f["name"] for f in features.values() if not f["required"] and not f["configured"]
    ]
    if optional_missing:
        logger.warning(
            f"Optional features not configured (predictions may be less accurate): {optional_missing}"
        )
