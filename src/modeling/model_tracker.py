...
class ModelTracker:
    ...
    def __init__(self, manifest_path: Optional[str] = None):
        ...
        if manifest_path is None:
            manifest_path = os.path.join(settings.models_dir, "manifest.json")
    ...
    def register_version(self, version: ModelVersion, set_active: bool = True) -> None:
        """
    ...
    def get_performance_history(self, model_id: str, days: Optional[int] = None) -> List[Dict]:
        ...
        if days:
            ...
            history = [p for p in history if datetime.fromisoformat(p["date"]).timestamp() > cutoff]

        return sorted(history, key=lambda x: x["date"])

    def compare_versions(self, version1: str, version2: str) -> Dict[str, float]:
        """
    ...
    def generate_report(self) -> str:
        ...
        for model_type, version in self.manifest["active_models"].items():
            ...
            if version_info:
                ...
                lines.append(
                    f"  {model_type:15} v{version:10} - Acc: {metrics.get('accuracy', 0):.1%}, LogLoss: {metrics.get('log_loss', 0):.4f}"
                )
        ...
        for model_type in self.manifest["active_models"].keys():
            ...
            if history:
                ...
                lines.append(
                    f"  {model_type:15} - Predictions: {len(history)}, Avg Acc: {avg_acc:.1%}, Avg ROI: {avg_roi:.1%}"
                )
            else:
