...
def get_timezone_difference(team1: str, team2: str) -> int:
    ...
    if tz1 == 0:
        for team_name, offset in TEAM_TIMEZONE_OFFSET.items():
            if (
                team_name.lower() in team1_normalized.lower()
                or team1_normalized.lower() in team_name.lower()
            ):
                tz1 = offset
                break
    ...
    if tz2 == 0:
        for team_name, offset in TEAM_TIMEZONE_OFFSET.items():
            if (
                team_name.lower() in team2_normalized.lower()
                or team2_normalized.lower() in team_name.lower()
            ):
                tz2 = offset
                break
