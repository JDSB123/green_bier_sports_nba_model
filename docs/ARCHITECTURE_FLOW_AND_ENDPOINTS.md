# Complete Architecture Flow & Endpoint Documentation

## 🏗️ System Architecture Overview

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                          SINGLE ENTRY POINT                                 │
│                    docker-entrypoint-backtest.sh                            │
│                         (Container Entrypoint)                              │
└────────────────────────────┬────────────────────────────────────────────────┘
                             │
                             ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│                         VALIDATION LAYER                                    │
│  ┌────────────────────────────────────────────────────────────────────┐    │
│  │ validate_env()                                                    │    │
│  │  ├─ API_BASKETBALL_KEY                                            │    │
│  │  └─ THE_ODDS_API_KEY                                              │    │
│  └────────────────────────────────────────────────────────────────────┘    │
│  ┌────────────────────────────────────────────────────────────────────┐    │
│  │ validate_python()                                                 │    │
│  │  ├─ Check: src.config, src.modeling.models                       │    │
│  │  ├─ Check: src.modeling.features                                 │    │
│  │  └─ Check: src.ingestion.api_basketball                           │    │
│  └────────────────────────────────────────────────────────────────────┘    │
└────────────────────────────┬────────────────────────────────────────────────┘
                             │
                             ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│                    DATA INGESTION PIPELINE                                  │
│              scripts/build_training_data_complete.py                           │
│                    FreshDataPipeline.build()                                │
└────────────────────────────┬────────────────────────────────────────────────┘
                             │
                             ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│  STEP 1: FETCH PARTICIPANTS (The Odds API)                                  │
│  ───────────────────────────────────────────────────────────────────────    │
│  Function: fetch_participants()                                             │
│  Endpoint: GET /v4/sports/basketball_nba/participants                       │
│  Purpose: Team reference data for validation                                │
│  ───────────────────────────────────────────────────────────────────────    │
│  Response: List of participant objects                                      │
│  Fields: name, id, key, etc.                                                │
│  Standardization: Team names normalized to ESPN format                      │
│  Status: ✅ OPTIMIZED (called at start)                                     │
└────────────────────────────┬────────────────────────────────────────────────┘
                             │
                             ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│  STEP 2: FETCH GAME OUTCOMES (API-Basketball)                               │
│  ───────────────────────────────────────────────────────────────────────    │
│  Function: FreshDataPipeline.fetch_game_outcomes()                          │
│  Client: APIBasketballClient.ingest_essential()                             │
│  ───────────────────────────────────────────────────────────────────────    │
│                                                                              │
│  ┌────────────────────────────────────────────────────────────────────┐    │
│  │ 2.1: FETCH TEAMS                                                   │    │
│  │ Endpoint: GET /v1/teams                                            │    │
│  │ Base URL: https://v1.basketball.api-sports.io                      │    │
│  │ Params: league=12, season={season}                                 │    │
│  │ ───────────────────────────────────────────────────────────────    │    │
│  │ Response: { response: [team objects] }                            │    │
│  │ Fields: id, name, code, logo, etc.                                 │    │
│  │ Output: data/raw/api_basketball/teams_*.json                       │    │
│  │ Purpose: Team reference (required for subsequent calls)            │    │
│  │ Status: ✅ OPTIMIZED                                               │    │
│  └────────────────────────────────────────────────────────────────────┘    │
│                                                                              │
│  ┌────────────────────────────────────────────────────────────────────┐    │
│  │ 2.2: FETCH GAMES                                                   │    │
│  │ Endpoint: GET /v1/games                                            │    │
│  │ Params: league=12, season={season}                                 │    │
│  │ ───────────────────────────────────────────────────────────────    │    │
│  │ Response: { response: [game objects] }                            │    │
│  │ Fields:                                                             │    │
│  │   - id, date, status.short (FT = finished)                         │    │
│  │   - teams.home.name, teams.away.name                               │    │
│  │   - scores.home.total, scores.away.total                           │    │
│  │   - scores.home.quarter_1, quarter_2, quarter_3, quarter_4         │    │
│  │   - scores.away.quarter_1, quarter_2, quarter_3, quarter_4         │    │
│  │ Output: data/raw/api_basketball/games_*.json                       │    │
│  │ Standardization: Team names → ESPN format (mandatory)               │    │
│  │ Purpose: Core game data with Q1-Q4 scores                           │    │
│  │ Status: ✅ OPTIMIZED (CORE ENDPOINT)                                │    │
│  └────────────────────────────────────────────────────────────────────┘    │
│                                                                              │
│  ┌────────────────────────────────────────────────────────────────────┐    │
│  │ 2.3: FETCH STATISTICS                                              │    │
│  │ Endpoint: GET /v1/statistics                                       │    │
│  │ Params: league=12, season={season}, team={team_id}                 │    │
│  │ ───────────────────────────────────────────────────────────────    │    │
│  │ Response: { response: {team statistics} }                         │    │
│  │ Fields:                                                             │    │
│  │   - games.minutes, games.points, games.fieldGoals                   │    │
│  │   - Points per game (PPG)                                           │    │
│  │   - Points allowed per game (PAPG)                                  │    │
│  │   - Win-Loss records (home/away)                                    │    │
│  │ Output: data/raw/api_basketball/statistics_*.json                   │    │
│  │ Purpose: Team PPG, PAPG, W-L (key features)                         │    │
│  │ Status: ✅ OPTIMIZED (TIER 1 ESSENTIAL)                             │    │
│  │ Note: Called per team (batch processing)                            │    │
│  └────────────────────────────────────────────────────────────────────┘    │
│                                                                              │
│  ┌────────────────────────────────────────────────────────────────────┐    │
│  │ 2.4: FETCH GAME STATISTICS (TEAMS)                                 │    │
│  │ Endpoint: GET /v1/games/statistics/teams                           │    │
│  │ Params: ids={game_id1-game_id2-game_id3...}                        │    │
│  │ ───────────────────────────────────────────────────────────────    │    │
│  │ Response: { response: [game stats objects] }                      │    │
│  │ Fields:                                                             │    │
│  │   - Full box scores per game                                        │    │
│  │   - Advanced stats (FG%, 3P%, rebounds, assists, etc.)             │    │
│  │ Output: data/raw/api_basketball/game_stats_teams_*.json            │    │
│  │ Purpose: Full box scores for advanced feature engineering           │    │
│  │ Status: ✅ OPTIMIZED (TIER 1 ESSENTIAL)                             │    │
│  │ Note: Batch processing (20 games per call)                          │    │
│  └────────────────────────────────────────────────────────────────────┘    │
│                                                                              │
│  Result: outcomes_df DataFrame with:                                        │
│    - date, home_team, away_team, home_score, away_score                    │
│    - home_q1, home_q2, home_q3, home_q4                                    │
│    - away_q1, away_q2, away_q3, away_q4                                    │
│    - game_id, season                                                        │
└────────────────────────────┬────────────────────────────────────────────────┘
                             │
                             ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│  STEP 3: FETCH BETTING LINES (The Odds API)                                 │
│  ───────────────────────────────────────────────────────────────────────    │
│  Function: FreshDataPipeline.fetch_betting_lines()                          │
│  ───────────────────────────────────────────────────────────────────────    │
│                                                                              │
│  Strategy: Try historical endpoint first (if paid plan),                    │
│            fallback to current odds with event-specific enrichment          │
│                                                                              │
│  ┌────────────────────────────────────────────────────────────────────┐    │
│  │ 3.1: FETCH HISTORICAL ODDS (if available)                          │    │
│  │ Endpoint: GET /v4/historical/sports/basketball_nba/odds            │    │
│  │ Base URL: https://api.the-odds-api.com                              │    │
│  │ Params:                                                              │    │
│  │   - apiKey={key}                                                    │    │
│  │   - regions=us                                                      │    │
│  │   - date={ISO date string}                                          │    │
│  │   - oddsFormat=american                                             │    │
│  │ ───────────────────────────────────────────────────────────────    │    │
│  │ Response: {                                                         │    │
│  │   data: [event objects],                                            │    │
│  │   timestamp: "...",                                                 │    │
│  │   previous_timestamp: "...",                                        │    │
│  │   next_timestamp: "..."                                             │    │
│  │ }                                                                   │    │
│  │ Fields per event:                                                   │    │
│  │   - id, sport_key, commence_time                                    │    │
│  │   - home_team, away_team                                            │    │
│  │   - bookmakers[].markets[]                                          │    │
│  │     • spreads: outcomes[].point (spread line)                       │    │
│  │     • totals: outcomes[].point (total line)                         │    │
│  │ Output: Cached in memory for processing                             │    │
│  │ Purpose: Historical FG markets for backtesting                      │    │
│  │ Status: ✅ OPTIMIZED (requires paid plan Group 2+)                  │    │
│  │ Note: Returns 403 if not enabled                                    │    │
│  └────────────────────────────────────────────────────────────────────┘    │
│                                                                              │
│  ┌────────────────────────────────────────────────────────────────────┐    │
│  │ 3.2: FETCH EVENT-SPECIFIC ODDS (1H Markets)                        │    │
│  │ Endpoint: GET /v4/sports/basketball_nba/events/{eventId}/odds      │    │
│  │ Params:                                                              │    │
│  │   - apiKey={key}                                                    │    │
│  │   - regions=us                                                      │    │
│  │   - markets=spreads_h1,totals_h1                             │    │
│  │   - oddsFormat=american                                             │    │
│  │ ───────────────────────────────────────────────────────────────    │    │
│  │ Response: Event object with bookmakers and markets                  │    │
│  │ Markets:                                                             │    │
│  │   - spreads_h1: First half spreads                                  │    │
│  │   - totals_h1: First half totals                                    │    │
│  │ Purpose: 1H markets (not in main /odds endpoint)                    │    │
│  │ Status: ✅ OPTIMIZED (called per event for enrichment)              │    │
│  │ Note: Merged with main event data                                   │    │
│  └────────────────────────────────────────────────────────────────────┘    │
│                                                                              │
│  ┌────────────────────────────────────────────────────────────────────┐    │
│  │ 3.3: FETCH EVENTS LIST (if historical not available)               │    │
│  │ Endpoint: GET /v4/sports/basketball_nba/events                     │    │
│  │ Params:                                                              │    │
│  │   - apiKey={key}                                                    │    │
│  │   - dateFormat=iso                                                  │    │
│  │ ───────────────────────────────────────────────────────────────    │    │
│  │ Response: Array of event objects                                    │    │
│  │ Fields: id, sport_key, commence_time, home_team, away_team         │    │
│  │ Purpose: Get event IDs for event-specific endpoint calls           │    │
│  │ Status: ✅ OPTIMIZED (fallback path)                                │    │
│  └────────────────────────────────────────────────────────────────────┘    │
│                                                                              │
│  ┌────────────────────────────────────────────────────────────────────┐    │
│  │ 3.4: FETCH CURRENT ODDS (fallback)                                 │    │
│  │ Endpoint: GET /v4/sports/basketball_nba/odds                       │    │
│  │ Params:                                                              │    │
│  │   - apiKey={key}                                                    │    │
│  │   - regions=us                                                      │    │
│  │   - oddsFormat=american                                             │    │
│  │ ───────────────────────────────────────────────────────────────    │    │
│  │ Response: Array of event objects with bookmakers                   │    │
│  │ Fields: Same as historical odds (current snapshot)                 │    │
│  │ Purpose: Current/recent odds when historical unavailable           │    │
│  │ Status: ✅ OPTIMIZED (fallback path)                                │    │
│  │ Note: Each event then enriched with 1H via event-specific          │    │
│  └────────────────────────────────────────────────────────────────────┘    │
│                                                                              │
│  Result: lines_df DataFrame with:                                           │
│    - query_date, event_id, commence_time                                    │
│    - home_team, away_team (standardized)                                    │
│    - fg_spread_line, fg_total_line, fg_home_ml, fg_away_ml                 │
│    - fh_spread_line, fh_total_line, fh_home_ml, fh_away_ml                 │
│    - 1h_spread_line, 1h_total_line, 1h_home_ml, 1h_away_ml                 │
└────────────────────────────┬────────────────────────────────────────────────┘
                             │
                             ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│  STEP 4: MERGE OUTCOMES + LINES                                             │
│  ───────────────────────────────────────────────────────────────────────    │
│  Function: FreshDataPipeline.merge_outcomes_and_lines()                     │
│  Logic: Match by team names (ESPN format) + date (within 1 day tolerance)   │
│  Result: merged_df with games + betting lines combined                      │
└────────────────────────────┬────────────────────────────────────────────────┘
                             │
                             ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│  STEP 5: COMPUTE LABELS                                                     │
│  ───────────────────────────────────────────────────────────────────────    │
│  Function: FreshDataPipeline.compute_labels()                               │
│  ───────────────────────────────────────────────────────────────────────    │
│  Computed Labels:                                                           │
│    - home_win = (home_score > away_score)                                   │
│    - spread_covered = (actual_margin > -spread_line)                        │
│    - total_over = (actual_total > total_line)                               │
│    - 1h_spread_covered = (actual_1h_margin > -1h_spread_line)              │
│    - 1h_total_over = (actual_1h_total > 1h_total_line)                      │
└────────────────────────────┬────────────────────────────────────────────────┘
                             │
                             ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│  STEP 6: ENRICH WITH BETTING SPLITS                                         │
│  ───────────────────────────────────────────────────────────────────────    │
│  Function: FreshDataPipeline.enrich_with_betting_splits()                   │
│  ───────────────────────────────────────────────────────────────────────    │
│                                                                              │
│  ┌────────────────────────────────────────────────────────────────────┐    │
│  │ 6.1: FETCH BETTING SPLITS (The Odds API - Primary)                 │    │
│  │ Endpoint: GET /v4/sports/basketball_nba/betting-splits             │    │
│  │ Params:                                                              │    │
│  │   - apiKey={key}                                                    │    │
│  │   - regions=us                                                      │    │
│  │ ───────────────────────────────────────────────────────────────    │    │
│  │ Response: Array of game objects with splits data                   │    │
│  │ Fields:                                                              │    │
│  │   - markets.spreads.choices[].value (public % on each side)        │    │
│  │   - markets.totals.choices[].value                                  │    │
│  │ Purpose: Public betting percentages, RLM detection                  │    │
│  │ Status: ✅ OPTIMIZED (tried first, requires paid plan Group 2+)     │    │
│  │ Note: Returns 403 if not enabled, falls back to Action Network     │    │
│  └────────────────────────────────────────────────────────────────────┘    │
│                                                                              │
│  ┌────────────────────────────────────────────────────────────────────┐    │
│  │ 6.2: FETCH BETTING SPLITS (Action Network - Premium + Public)       │    │
│  │ Endpoint: POST https://api.actionnetwork.com/web/v1/auth/login     │    │
│  │          GET https://api.actionnetwork.com/web/v1/games/nba        │    │
│  │          GET https://api.actionnetwork.com/web/v1/scoreboard/nba   │    │
│  │ Params:                                                              │    │
│  │   - ACTION_NETWORK_USERNAME (optional - enables premium access)     │    │
│  │   - ACTION_NETWORK_PASSWORD (optional - enables premium access)     │    │
│  │ ───────────────────────────────────────────────────────────────    │    │
│  │ Response: JSON with betting splits                                  │    │
│  │ Fields: Public percentages, ticket/money splits, expert picks       │    │
│  │ Purpose: Primary betting splits source (premium when available)     │    │
│  │ Status: ✅ ENHANCED (premium auth + public fallback)               │    │
│  │ Note: Premium access provides Action PRO/Labs features              │    │
│  └────────────────────────────────────────────────────────────────────┘    │
│                                                                              │
│  Added Features:                                                             │
│    - spread_public_home_pct, spread_ticket_money_diff                       │
│    - is_rlm_spread (reverse line movement indicator)                        │
│    - over_public_pct, total_ticket_money_diff                               │
│    - sharp_side_spread, sharp_side_total                                    │
└────────────────────────────┬────────────────────────────────────────────────┘
                             │
                             ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│  STEP 7: VALIDATE DATASET                                                   │
│  ───────────────────────────────────────────────────────────────────────    │
│  Function: FreshDataPipeline.validate_dataset()                             │
│  Checks:                                                                    │
│    - Required columns exist                                                 │
│    - Data quality (null percentages < 20%)                                  │
│    - Spread line coverage (>10%)                                            │
│    - Total line coverage (>10%)                                             │
│  Result: Validated training_data.csv                                        │
└────────────────────────────┬────────────────────────────────────────────────┘
                             │
                             ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│  STEP 8: SAVE TRAINING DATA                                                 │
│  ───────────────────────────────────────────────────────────────────────    │
│  Output: data/processed/training_data.csv                                   │
│  Columns: ~100+ columns including:                                          │
│    - Core: date, home_team, away_team, home_score, away_score              │
│    - Quarters: home_q1-q4, away_q1-q4                                       │
│    - Labels: spread_covered, total_over, 1h_spread_covered, etc.           │
│    - Lines: spread_line, total_line, fh_spread_line, fh_total_line, etc.   │
│    - Features: (will be computed during backtest)                           │
└────────────────────────────┬────────────────────────────────────────────────┘
                             │
                             ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│  STEP 9: RUN BACKTEST                                                       │
│  ───────────────────────────────────────────────────────────────────────    │
│  Script: scripts/backtest_production.py                                                │
│  ───────────────────────────────────────────────────────────────────────    │
│                                                                              │
│  ┌────────────────────────────────────────────────────────────────────┐    │
│  │ 9.1: LOAD TRAINING DATA                                            │    │
│  │ Function: load_training_data()                                     │    │
│  │ Input: data/processed/training_data.csv                            │    │
│  │ Output: DataFrame sorted by date                                   │    │
│  └────────────────────────────────────────────────────────────────────┘    │
│                                                                              │
│  ┌────────────────────────────────────────────────────────────────────┐    │
│  │ 9.2: WALK-FORWARD VALIDATION                                       │    │
│  │ For each market (fg_spread, fg_total, 1h_spread, etc.):           │    │
│  │                                                                     │    │
│  │   For game i in range(min_training, total_games):                  │    │
│  │     ├─ Training data: games[0:i]                                   │    │
│  │     ├─ Test game: games[i]                                         │    │
│  │     │                                                               │    │
│  │     └─ Build features via FeatureEngineer:                         │    │
│  │         ├─ compute_team_rolling_stats()                            │    │
│  │         │   └─ Historical games only (< game date)                 │    │
│  │         ├─ compute_rest_days()                                     │    │
│  │         ├─ compute_travel_features()                               │    │
│  │         ├─ compute_dynamic_hca()                                   │    │
│  │         ├─ compute_sos_features()                                  │    │
│  │         └─ build_game_features() → feature dict                    │    │
│  │                                                                     │    │
│  │     ├─ Train model on training features                            │    │
│  │     │   └─ Model.fit(X_train, y_train)                             │    │
│  │     │                                                               │    │
│  │     ├─ Predict on test game                                        │    │
│  │     │   └─ Model.predict_proba(X_test) → probabilities             │    │
│  │     │                                                               │    │
│  │     └─ Calculate accuracy/ROI                                      │    │
│  │         └─ Compare prediction vs actual outcome                    │    │
│  └────────────────────────────────────────────────────────────────────┘    │
│                                                                              │
│  ┌────────────────────────────────────────────────────────────────────┐    │
│  │ 9.3: SAVE RESULTS                                                  │    │
│  │ Output:                                                             │    │
│  │   - data/processed/all_markets_backtest_results.csv                │    │
│  │   - ALL_MARKETS_BACKTEST_RESULTS.md                                │    │
│  └────────────────────────────────────────────────────────────────────┘    │
└─────────────────────────────────────────────────────────────────────────────┘
```

---

## 📡 Complete Endpoint Reference

### API-Basketball (https://v1.basketball.api-sports.io)

| Endpoint | Method | Purpose | Status | Params |
|----------|--------|---------|--------|--------|
| `/v1/teams` | GET | Team reference | ✅ **OPTIMIZED** | `league=12, season={season}` |
| `/v1/games` | GET | Game outcomes + Q1-Q4 | ✅ **OPTIMIZED** | `league=12, season={season}` |
| `/v1/statistics` | GET | Team PPG/PAPG/W-L | ✅ **OPTIMIZED** | `league=12, season={season}, team={id}` |
| `/v1/games/statistics/teams` | GET | Full box scores | ✅ **OPTIMIZED** | `ids={id1-id2-id3...}` |
| `/v1/standings` | GET | Conference rankings | ⚠️ Optional | `league=12, season={season}` |
| `/v1/games/statistics/players` | GET | Player box scores | ⚠️ Optional | `ids={id1-id2-id3...}` |
| `/v1/players` | GET | Team rosters | ⚠️ Optional | `team={id}, season={season}` |

**Tier 1 (Essential - Always Called):**
- ✅ `/teams` - Base reference
- ✅ `/games` - Core outcomes
- ✅ `/statistics` - Team stats
- ✅ `/games/statistics/teams` - Box scores

**Tier 2 (Valuable - Can be added):**
- ⚠️ `/standings` - Rankings
- ⚠️ `/games/statistics/players` - Player stats

**Tier 3 (Reference - Static):**
- ⚠️ `/players` - Rosters
- ⚠️ `/bookmakers` - Sportsbook names
- ⚠️ `/bets` - Bet type definitions

---

### The Odds API (https://api.the-odds-api.com)

| Endpoint | Method | Purpose | Status | Params |
|----------|--------|---------|--------|--------|
| `/v4/sports/basketball_nba/participants` | GET | Team reference | ✅ **OPTIMIZED** | `apiKey, dateFormat=iso` |
| `/v4/sports/basketball_nba/events/{eventId}/odds` | GET | 1H markets | ✅ **OPTIMIZED** | `apiKey, regions=us, markets=spreads_h1,totals_h1` |
| `/v4/sports/basketball_nba/events` | GET | Events list | ✅ **OPTIMIZED** | `apiKey, dateFormat=iso` |
| `/v4/sports/basketball_nba/scores` | GET | Recent scores | ⚠️ Not used | `apiKey, daysFrom=1` |
| `/v4/sports/basketball_nba/betting-splits` | GET | Public betting % | ✅ **OPTIMIZED** | `apiKey, regions=us` |

**Primary Endpoints (Always Called):**
- ✅ `/participants` - Team validation
- ✅ `/historical/.../odds` or `/odds` - FG markets
- ✅ `/events/{id}/odds` - 1H enrichment
- ✅ `/betting-splits` - Public percentages

**Optional Endpoints:**
- ⚠️ `/scores` - Not used (we use API-Basketball for scores)
- ⚠️ `/events` - Only used when historical unavailable

---

## 🔄 Complete Data Flow

### Input Data Sources

```
┌──────────────────────┐         ┌──────────────────────┐
│  API-Basketball      │         │   The Odds API       │
│                      │         │                      │
│  • Game outcomes     │         │  • Betting lines     │
│  • Q1-Q4 scores      │         │  • Spreads/totals    │
│  • Team statistics   │         │  • 1H markets        │
│  • Box scores        │         │  • Betting splits    │
└──────────┬───────────┘         └──────────┬───────────┘
           │                                 │
           └─────────────┬───────────────────┘
                         │
                         ▼
              ┌──────────────────────┐
              │  Standardization     │
              │  (ESPN format)       │
              └──────────┬───────────┘
                         │
                         ▼
              ┌──────────────────────┐
              │  Merge & Enrich      │
              │  (Match by teams)    │
              └──────────┬───────────┘
                         │
                         ▼
              ┌──────────────────────┐
              │  training_data.csv   │
              │  (Complete dataset)  │
              └──────────┬───────────┘
                         │
                         ▼
              ┌──────────────────────┐
              │  Feature Engineering │
              │  (Historical stats)  │
              └──────────┬───────────┘
                         │
                         ▼
              ┌──────────────────────┐
              │  Model Training      │
              │  (Walk-forward)      │
              └──────────┬───────────┘
                         │
                         ▼
              ┌──────────────────────┐
              │  Predictions         │
              │  (Probabilities)     │
              └──────────────────────┘
```

---

## 📊 Feature Engineering Flow

```
For each game in backtest:
│
├─ Historical Data (all games < current game date)
│
├─ Build Features:
│  ├─ Team Rolling Stats (last 10 games)
│  │  ├─ PPG, PAPG, margin, win_pct
│  │  ├─ Home/away splits
│  │  ├─ Form (last 3 vs last 10)
│  │  ├─ Clutch performance
│  │  ├─ Opponent-adjusted metrics
│  │  └─ Consistency metrics
│  │
│  ├─ Context Features
│  │  ├─ Rest days (home/away)
│  │  ├─ Back-to-back indicators
│  │  ├─ Travel distance/fatigue
│  │  ├─ Timezone changes
│  │  └─ Dynamic HCA
│  │
│  ├─ Head-to-Head
│  │  ├─ H2H margin (last 5 games)
│  │  ├─ H2H win percentage
│  │  ├─ Recent H2H margin
│  │  └─ H2H total average
│  │
│  ├─ Strength of Schedule
│  │  ├─ SOS rating
│  │  ├─ Recent SOS
│  │  └─ Opponent averages
│  │
│  ├─ Market Features
│  │  ├─ Spread/total lines
│  │  ├─ Line movement
│  │  ├─ RLM indicators
│  │  └─ Sharp money signals
│  │
│  ├─ Predictions
│  │  ├─ predicted_margin (FG)
│  │  ├─ predicted_total (FG)
│  │  ├─ predicted_margin_1h (1H)
│  │  └─ predicted_total_1h (1H)
│  │
│  └─ Injury Features (when available)
│     ├─ Injury spread impact
│     └─ Star player out indicators
│
└─ Output: Feature dictionary → Model prediction
```

---

## ✅ Endpoint Usage Summary

### Always Called (Essential)
1. **API-Basketball:**
   - `/teams` - 1 call per season
   - `/games` - 1 call per season
   - `/statistics` - 30 calls per season (1 per team)
   - `/games/statistics/teams` - ~25-30 calls per season (batched)

2. **The Odds API:**
   - `/participants` - 1 call at start
   - `/historical/.../odds` or `/odds` - 1 call per date
   - `/events/{id}/odds` - N calls (1 per event for 1H)
   - `/betting-splits` - 1 call (if available)

### Optional (Fallback/Enhancement)
- `/events` - Only if historical unavailable
- Action Network splits - Only if The Odds API splits unavailable

---

## 🔍 Verification: How to Confirm All Endpoints Are Used

**Check logs for these messages:**

```
✓ Fetched 30 participants for team validation
Ingesting: teams
Ingesting: games
Ingesting: statistics
Ingesting: game_stats_teams
Fetching betting lines for 180 unique dates...
✓ Historical odds endpoint available
✓ Fetched event odds for event {id} (1H markets)
✓ Fetched betting splits for 50 games
```

**Count API calls:**
- API-Basketball: ~60-65 calls per season (teams=1, games=1, stats=30, game_stats=~30)
- The Odds API: ~180 calls for dates + N calls for events (1H) + 1 for splits

---

## 📝 Summary

**Total Endpoints Used:** 12+ endpoints across 2 APIs

**API-Basketball:** 4 essential endpoints (Tier 1) always called
**The Odds API:** 5 endpoints (participants, odds, events, event-odds, splits)

**All endpoints are now optimized and being used in the container pipeline!** ✅
