...
Usage:
    ...
    python scripts/manage_models.py promote --name xgboost_model --version 1.0.0

    # Compare two models
    python scripts/manage_models.py compare --model1 xgboost_model:1.0.0 --model2 xgboost_model:1.1.0
...
def promote_model(args):
    ...
    if not target:
        print(f"Error: Model {args.name} v{args.version} not found in registry.")
        print("\nAvailable models:")
        for m in models:
    ...
    result = registry.promote_to_production(args.name, args.version, promoted_by)
...
def compare_models(args):
    ...
    if comparison["differences"]:
        ...
        for metric, data in comparison["differences"].items():
            ...
            rows.append([
                metric,
                f"{data['model1']:.4f}" if isinstance(data['model1'], float) else data['model1'],
                f"{data['model2']:.4f}" if isinstance(data['model2'], float) else data['model2'],
                f"{data['difference']:+.4f}" if isinstance(data['difference'], float) else data['difference'],
                pct_str,
            ])
...
def main():
    ...
    # Production command
    prod_parser = subparsers.add_parser("production", help="Show current production model")
